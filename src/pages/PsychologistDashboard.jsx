...
export default function PsychologistDashboard() {
  ...
  useEffect(() => {
    if (user?.role === "psychologist" && user?.profile_completed === false) {
      navigate("/EditProfile")
    }
  }, [user?.role, user?.profile_completed, navigate])
  ...
  const { data: resources = [] } = useQuery({
    ...
    queryFn: () => base44.entities.Resource.filter({ created_by: user?.email }),
    enabled: !!user?.email,
  })

  const upcoming = appointments.filter((a) => a.status === "upcoming")
  const completed = appointments.filter((a) => a.status === "completed")
  const uniquePatients = [...new Set(appointments.map((a) => a.created_by).filter(Boolean))]
