...
export default function PsychologistResources() {
  ...
  const { data: resources = [], isLoading } = useQuery({
    queryKey: ["myResources", currentUser?.id],
    queryFn: () => base44.entities.Resource.filter({ created_by: currentUser?.id }, "-created_date", 200),
      ...
      <div className="max-w-3xl mx-auto px-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <Link to="/Resources" className="text-sm text-[#E8907C] hover:underline mt-1 inline-block">
              Browse public resource library →
            </Link>
          </div>
          <Button
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    ...
                    <Link
                      ...
                      state={{ from: "psych" }}
                      className="font-semibold text-gray-800 hover:text-[#E8907C] block"
                    >
                      {r.title}
                    </Link>
