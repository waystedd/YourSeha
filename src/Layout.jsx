...
import {
  Home, Users, UserCircle, BookOpen, PenLine, Bell, AlertTriangle,
  Menu, ChevronDown, Globe, MessageCircle, BarChart2, Calendar, Settings, Mail
} from 'lucide-react';
...
function LayoutInner({ children, currentPageName }) {
  ...
  useEffect(() => {
    const allowed = ['/EditProfile', '/SignIn', '/RoleSelection'];
  ...
  const psychologistNavItems = [
    ...
    { name: 'Messages', page: 'Inbox', icon: MessageCircle },
    { name: 'Edit Profile', page: 'EditProfile', icon: Settings },
  ];

  const navItems = isPsychologist ? psychologistNavItems : caregiverNavItems;
      ...
      <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-[#FFE5D9]/50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <Link to={isPsychologist ? createPageUrl('PsychologistDashboard') : createPageUrl('Home')} className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <DropdownMenuContent align="end" className="w-56 rounded-2xl border-[#FFE5D9]">
                    ...
                    <Link to={createPageUrl('RoleSelection')}>
                      <DropdownMenuItem className="cursor-pointer text-blue-500">
                        <UserCircle className="w-4 h-4 mr-2" /> Switch Role
                      </DropdownMenuItem>
                    </Link>
                  ...
                  <div className="flex flex-col gap-2 mt-8">
                    ...
                    {!user ? (
                      <Link
                        to={createPageUrl('SignIn')}
                        onClick={() => setMobileMenuOpen(false)}
                        className="flex items-center justify-center gap-2 mt-2 px-4 py-3 rounded-xl bg-gradient-to-r from-[#F4A896] to-[#E8907C] text-white font-medium"
                      >
                        Sign In
                      </Link>
                    ) : (
